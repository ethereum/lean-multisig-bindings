...
not a supported or versioned general-purpose C API. `scripts/stage-static.sh` uses
`cargo zigbuild` to build and stage one explicit Rust target's static archive for Go builds and
release packaging.
