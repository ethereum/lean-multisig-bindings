...
require_text "$pr_workflow" "if: contains\(github\.event\.pull_request\.labels\.\*\.name, 'benchmark-slow'\) && \(github\.event\.action != 'labeled' \|\| github\.event\.label\.name == 'benchmark-slow'\)" 'slow label gate'
